...
For example a MOVE REQUEST can be received from the server, the function can call another function on the client side to provide a move. When a message is
received a signal corresponding to that message is emitted. This will trigger a slot on the client side to implement client side logic and update the GUI
 */

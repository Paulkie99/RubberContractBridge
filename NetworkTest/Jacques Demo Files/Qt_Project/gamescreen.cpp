...
void GameScreen::on_pushButton_Play_clicked()
{
    ...
    QString mes2 = clientgs->GenerateMessage("SCORE");
    QJsonObject mesobj = clientgs->CreateJObject(mes2);
    scoreSlot(mesobj);
}
...
//This slot is used to handle the client side logic for when a score update is received from the server.
//This function extracts the scores of the two teams from the received Jsonobject and calls the updateScores
//function of the scoreboard to update the score. At the moment the scoreboard is created from scratch each time in this function.
